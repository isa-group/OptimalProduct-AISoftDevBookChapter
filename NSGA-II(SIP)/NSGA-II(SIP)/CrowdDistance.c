/* Crowding distance computation routines of NSGA-II*/
...
void assign_crowding_distance_list (population *pop, list *lst, int front_size)
{
    ...
    obj_array = (int **)malloc(nsobj*sizeof(int));
...
void assign_crowding_distance (population *pop, int *dist, int **obj_array, int front_size)
{
	...
	for (i=0; i<nsobj; i++)
    {
        for (j=1; j<front_size-1; j++)
        {
            ...
            {
                if (pop->ind[obj_array[i][front_size-1]].sobj[i] == pop->ind[obj_array[i][0]].sobj[i])
                {
                    pop->ind[obj_array[i][j]].crowd_dist += 0.0;
                }
                else
                {
                    pop->ind[obj_array[i][j]].crowd_dist += (pop->ind[obj_array[i][j+1]].sobj[i] - pop->ind[obj_array[i][j-1]].sobj[i])/(pop->ind[obj_array[i][front_size-1]].sobj[i] - pop->ind[obj_array[i][0]].sobj[i]);
                }
            }
        }
    }
    for (j=0; j<front_size; j++)
    {
        ...
        {
            pop->ind[dist[j]].crowd_dist = (pop->ind[dist[j]].crowd_dist)/nsobj;
        }
    }
    return;
}
